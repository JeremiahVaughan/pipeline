...
fn validate_config(config: &AppConfig) -> Result<(), String> {
    ...
    for (repo_name, repo_cfg) in &config.repos {
        match repo_cfg {
            RepoConfig::Clone { vcs, clone } => {
                if vcs.is_empty() {
                    return Err(format!("repo {repo_name} requires a vcs"));
                }
                if clone.url.is_empty() {
                    return Err(format!("repo {repo_name} requires a clone.url"));
                }
                if clone.dir.is_empty() {
                    return Err(format!("repo {repo_name} requires a clone.dir"));
                }
                if clone.db_file.is_empty() {
                    return Err(format!("repo {repo_name} requires a clone.db_file"));
                }
            }
            RepoConfig::Direct {
                vcs,
